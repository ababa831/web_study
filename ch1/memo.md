...
- RESTful (REpresentational State Transfer)
    - ４つの原則からなるシンプルな設計
        - 統一インターフェース 予め定義・共有された方法(e.g. HTTP)で情報がやりとり
        - アドレス可能性 全ての情報が一意なURL構文で示される
        - 接続性 やりとりする情報にリンクを含められる
        - ステートレス性 やりとりは１回ごとに完結．前の状態（ステート）を保持しない（レス）
    - ↑の原則により，シンプルで統一的，別の情報をリンク可能なため，RESTfulなシステム同士であれば円滑な情報連携が可能
    - 多くのWebアプリケーションがRESTfulに設計されている
- セマンティックWeb
    - Webページの情報に意味（セマンティック）を付加したもの
    - コンピュータが自律的に情報の意味を理解して，処理できるようになることを期待
    - 言語 
        - XMLでWebページを構成
        - XML中にRDFという言語で意味を記述
        - 言葉の相互関係をOWLという言語で記述
        - ↑のような「情報の意味」を示す情報をメタデータと呼ぶ
    - W3Cで標準化が進められている
    - 恩恵
        - 情報検索の精度向上
            - e.g. 「長野」が苗字なのか地名なのかという情報を持たせて，検索精度向上
        - 特定の種類の情報を集めて活用
    - 課題 メタデータの付与作業量が馬鹿にならない．普及は先

## Column Webアプリケーション，Webシステム　用語のちがい

- Webシステム Webサイト，アプリケーション，サービスを提供するための仕組み
    - Webページ Web上にある文書
        - Webサイト 特定のドメインの下にあるWebページに集まり
        - ホームページ Webブラウザを起動したときに最初に開かれるWebページ
    - Webアプリケーション Webを介して**人**が利用するサービス
    - Webサービス Webを介して**プログラム**が利用するサービス